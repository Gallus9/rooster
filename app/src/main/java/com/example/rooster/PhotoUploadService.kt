...
class PhotoUploadService(
    ...
    private suspend fun startPeriodicSync() {
        while (uploadScope.isActive) {
            try {
                ...
                val syncInterval = when (networkQuality) {
    ...
    private suspend fun processQueuedUploadsForNetwork(networkQuality: NetworkQualityLevel) {
        ...
        val maxConcurrent = when (networkQuality) {
    ...
    suspend fun enqueueUpload(requestDto: SerializablePhotoUploadRequest) {
        val entity = requestDto.toPhotoUploadEntity()
    ...
    suspend fun resumeQueuedUploads() {
        ...
        val networkQuality = networkQualityManager.getCurrentNetworkQuality()
        val maxToResume = when (networkQuality) {
            NetworkQualityLevel.POOR, NetworkQualityLevel.OFFLINE -> max2GConcurrentUploads
            else -> maxGoodNetworkConcurrentUploads
        }
    ...
    private fun processUpload(
        ...
        val job = uploadScope.launch {
            ...
            try {
                ...
                val saveSuccessful = suspendCancellableCoroutine<Boolean> { continuation ->
                    val timeoutJob = uploadScope.launch {
                        val networkQuality = networkQualityManager.getCurrentNetworkQuality()
                        val timeout = when (networkQuality) {
    ...
    private suspend fun handleUploadFailureEnhanced(
        ...
        if (entity.retryCount < maxRetryCount) {
            ...
            val networkQuality = networkQualityManager.getCurrentNetworkQuality()
            val delayTime = when (networkQuality) {
    ...
    private suspend fun linkToTargetObject(
        ...
        try {
            ...
            val linkSuccessful = suspendCancellableCoroutine<Boolean> { continuation ->
                targetObject.saveInBackground(object : SaveCallback {
                    override fun done(e: ParseException?) {
                        if (continuation.isActive) {
                            if (e == null) {
                                continuation.resume(true) {}
    ...
    private suspend fun fetchParseObject(className: String, objectId: String): ParseObject? {
        return try {
    ...
    private fun get2GOptimizedImageBytes(uri: Uri, fileName: String): ByteArray? {
        return try {
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                ...
                val networkQuality = networkQualityManager.getCurrentNetworkQuality()
                val compressionConfig = when (networkQuality) {
                    NetworkQualityLevel.POOR -> {
                ...
                val resizedBitmap = if (originalBitmap.width > compressionConfig.maxDimension || 
            ...
            FirebaseCrashlytics.getInstance().recordException(RuntimeException("2G image compression failed for $fileName: ${e.message}", e))
            null
        }
    }
